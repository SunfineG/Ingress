...
template <typename FRAG_T, typename LINE_PARSER_T = TSVLineParser<
                               ...
                               typename FRAG_T::edata_t>>
static void LoadChangeEFile(
    std::shared_ptr<typename FRAG_T::vertex_map_t>& vm_ptr,
    const std::string& efile_update,
    std::vector<std::pair<typename FRAG_T::oid_t, typename FRAG_T::oid_t>>&
        added_edges,
    std::vector<std::pair<typename FRAG_T::oid_t, typename FRAG_T::oid_t>>&
        deleted_edges) {
  ...
  std::ifstream edegfile(efile_update);
  std::string line, type;
  auto fid = vm_ptr->GetCommSpec().fid();
  vid_t src, dst;
  vid_t dummy;

  if (!edegfile.is_open()) {
    LOG(FATAL) << "Failed to open: " << efile_update;
  }
  ...
  while (getline(edegfile, line)) {
    ...
    if (type == "a") {
      if (vm_ptr->GetGid(fid, src, dummy)) {
        added_edges.template emplace_back(src, dst);
        CHECK(vm_ptr->GetGid(dst, dummy));
      }
    } else if (type == "d") {
      if (vm_ptr->GetGid(fid, src, dummy)) {
        deleted_edges.template emplace_back(src, dst);
        CHECK(vm_ptr->GetGid(dst, dummy));
      }
    } else {
