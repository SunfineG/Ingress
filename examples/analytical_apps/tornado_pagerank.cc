...
int main(int argc, char* argv[]) {
  ...
  grape::gflags::SetUsageMessage(
      "Usage: mpiexec [mpi_opts] ./tornado_pagerank [grape_opts]");
  if (argc == 1) {
  ...
  std::string efile_updated = FLAGS_efile_updated;
  ...
  using AppType = grape::tornado::PageRank<GraphType>;

  grape::tornado::CreateAndQuery<GraphType, AppType>(
      comm_spec, efile, efile_updated, vfile, out_prefix, spec, comm_spec,
      FLAGS_pr_d, FLAGS_pr_tol);
