...
def get_dataset():
    ...
    dynamic_dataset = {'dewiki.JAN': source,
                       ...
                       'dewiki.APR': source,
                       'dewiki.MAY': source}
    varg_dataset = {'2_road_usa': 16651563,
                    '4_road_usa': 16651563,
                    '6_road_usa': 16651563,
                    '8_road_usa': 16651563}
    if len(sys.argv) == 3:
...
def get_prefix(name: str):
    ...
    elif "_road_usa" in name:
        return "{PREFIX}/varg".format(PREFIX=dataset_prefix, NAME=name, PERCENTAGE=percentage)
    else:
...
def get_vfile(name: str):
    if "dewiki" in name:
        ...
        return "{PREFIX}/varg/{NAME}_w.v".format(PREFIX=dataset_prefix, NAME=name)
    else:
...
def run_tornado():
    ...
    for app in ('tornado_pagerank', 'tornado_php'):
        table = [["Batch(s)"], ["Inc(s)"], ["Mem(MB)"]]
